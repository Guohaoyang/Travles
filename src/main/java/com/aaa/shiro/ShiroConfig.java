...
public class ShiroConfig {
    ...
    public ShiroFilterFactoryBean getShiroFilterFactoryBean(@Qualifier("securityManager") DefaultWebSecurityManager manager){
         ...
         *      anon: 无需认证（登录）可以访问
         *      authc: 必须认证才可以访问
         *      user: 如果使用rememberMe功能可以直接访问就是记住我
    ...
    //先注释    filerMap.put("/strategy/a","authc");//这是需要登录才能访问的y
        ...
        //设置登录的页面，发送toLogin请求
        bean.setLoginUrl("/user/a");

       //设置未授权的页面
        bean.setUnauthorizedUrl("/user/a");
